...
!!! tip "Multiple sync providers"
    Gist sync can run alongside Cloud sync, [Cloudflare sync](cloudflare-sync.md), [S3 sync](s3-sync.md) and other sync provider plugins — Voltius shows each one separately in the sync menu and title bar. See [Building a sync provider](../plugins/sync-providers.md).
